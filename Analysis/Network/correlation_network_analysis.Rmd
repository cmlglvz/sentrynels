...
ppe <- read.table("/Documents/Proyectos_R/sentrynels/Data/Network/ppe.csv", sep = ";", h = FALSE)
core <- read.table("/Documents/Proyectos_R/sentrynels/Data/Network/core.csv", sep = ";", h = FALSE)
...
for (i in 1:dim(path_u)[1]) {
  ...
  node_t_class[[i]] <- node_t[[i]] %>% 
    dplyr::mutate("Node_Category" = case_when(Node %in%  t(ulist[[i]])[,1]~"Unique", Node %in% t(core[1,-1])~"Shared", Node %in% t(ppe[1,-1])~"Core", TRUE~"Other"))
...
par(mfrow = c(2,3))
ASVs <- data.frame("Other" = NA, "PPE" = NA, "Core" = NA, "Unique" = NA)
for (i in 1:6) {
...
pal <- c("#9CAFB7", "#2B2D42", "#02748A" , "#EF233C")
par(mfrow = c(2,3))
for (x in 1:6) {
  set.seed(40000)
  plot(graphlist[[x]], edge.color = "#D9801C", vertex.label = NA, 
       vertex.color = case_when(vertex_attr(graphlist[[x]], "Node_Category") == "Other"~pal[2], 
                                vertex_attr(graphlist[[x]], "Node_Category") == "PPE"~pal[3], 
                                vertex_attr(graphlist[[x]], "Node_Category") == "Unique"~pal[1], 
                                vertex_attr(graphlist[[x]], "Node_Category") == "Core"~pal[4]), 
...
degs <- degs %>% bind_rows()
deg2 <- deg %>% bind_rows()
ggplot(degs, aes(y = Total, x = Net, fill = factor(Var1, levels = c("Other", "Unique", "PPE", "Core")))) + 
  geom_col() + 
  scale_fill_manual(name = "ASV Category", values = c("#9CAFB7", "#2B2D42", "#02748A" , "#EF233C"))
ggplot(deg2, aes(y = Total, x = Net, fill = factor(Var1, levels = c("Other", "Unique", "PPE", "Core")))) + 
  geom_col() + 
  scale_fill_manual(name = "ASV Category", values = c("#9CAFB7", "#2B2D42", "#02748A" , "#EF233C"))
nods <- nod
nods[[4]] <- NULL
nods <- nod %>% bind_rows()
nod2 <- nod %>% bind_rows()
ggplot(nods, aes(y = Total, x = Net, fill = factor(Var1, levels = c("Other", "Unique", "PPE", "Cores")))) + 
  geom_col() + 
  scale_fill_manual(name = "ASV Category", values = c("#9CAFB7", "#2B2D42", "#02748A" , "#EF233C"))
ggplot(nod2, aes(y = Total, x = Net, fill = factor(Var1, levels = c("Other", "Unique", "PPE", "Core")))) + 
  geom_col() + 
  scale_fill_manual(name = "ASV Category", values = c("#9CAFB7", "#2B2D42", "#02748A" , "#EF233C"))

ggplot(nod2, aes(y = Freq, x = Net, fill = Freq)) + geom_col()
